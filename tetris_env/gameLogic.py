...
def drawBox(boxx, boxy, color, pixelx=None, pixely=None):
    # Draw a single box at board or pixel coordinates
    if color == BLANK:  # Skip blank boxes
        return
    if not isinstance(color, int) or color < 0 or color >= len(COLORS):
        print(f"Warning: Invalid color value '{color}'. Using default color.")
        color = 0  # Default to the first color (e.g., black or empty)

    if pixelx is None and pixely is None:
        pixelx, pixely = convertToPixelCoords(boxx, boxy)

    # Draw the box with both the main color and the highlight color
...
def drawStatus(score, lines, level, brain):
    ...
    titleSurf = BASICFONT.render('Youssef Elkhayat, CS4300 Final Project:', True, TEXTCOLOR)
